...
class ReciprocalLatticeViewer {
    constructor(exptParser, reflParser){
        ...
        this.hightlightColor = new _three.Color(ReciprocalLatticeViewer.colors()["highlight"]);
        this.reflectionUnindexedColor = new _three.Color(ReciprocalLatticeViewer.colors()["reflectionObsUnindexed"]);
        this.reflectionInexedColor = new _three.Color(ReciprocalLatticeViewer.colors()["reflectionObsIndexed"]);
        this.reflectionCalculatedColor = new _three.Color(ReciprocalLatticeViewer.colors()["reflectionCal"]);
        this.rlpScaleFactor = 1000;
        this.reflSprite = new _three.TextureLoader().load((0, _discPngDefault.default));
    ...
    updateReciprocalCell(val = null) {
        ...
        if (val !== null) this.reciprocalCellCheckbox.checked = val;
        for(var i = 0; i < this.reciprocalCellMeshes.length; i++)this.reciprocalCellMeshes[i].visible = this.reciprocalCellCheckbox.checked;
        if (this.reciprocalCellCheckbox.checked) this.zoomInOnObject(this.reciprocalCellMeshes[0]);
        this.requestRender();
    }
    updateReflectionSize() {
        ...
        if (this.refl.containsXYZObs()) {
            if (this.reflPositionsUnindexed) {
                const pointsObs = this.createPoints(this.reflPositionsUnindexed, ReciprocalLatticeViewer.colors()["reflectionObsUnindexed"], this.reflectionSize.value);
            ...
            if (this.reflPositionsIndexed) {
                const pointsObs = this.createPoints(this.reflPositionsIndexed, ReciprocalLatticeViewer.colors()["reflectionObsIndexed"], this.reflectionSize.value);
        ...
        if (this.refl.containsXYZCal() && this.reflPositionsCal) {
            const pointsCal = this.createPoints(this.reflPositionsCal, ReciprocalLatticeViewer.colors()["reflectionCal"], this.reflectionSize.value);
    ...
    getS1(point, dMatrix, wavelength, scaleFactor = [
        1,
        1
    ]) {
        const point3 = new _three.Vector3(point[0] * scaleFactor[0], point[1] * scaleFactor[1], 1.0);
    ...
    createPoints(positions, color, size) {
        const reflGeometry = new _three.BufferGeometry();
        reflGeometry.setAttribute("position", new _three.Float32BufferAttribute(positions, 3));
        const reflMaterial = new _three.PointsMaterial({
            size: size,
            map: this.reflSprite,
            alphaTest: 0.5,
            transparent: true,
        ...
        return new _three.Points(reflGeometry, reflMaterial);
    }
    showCloseReflButton() {
    ...
    addReflections() {
        function getRLP(s1, wavelength, unitS0, viewer1, goniometer, angle, U) {
            const rlp = s1.clone().normalize().sub(unitS0.clone().normalize()).multiplyScalar(1 / wavelength);
            if (goniometer == null) return rlp.multiplyScalar(viewer1.rlpScaleFactor);
            if (angle == null) {
                console.warn("Rotation angles not in reflection table. Cannot generate rlps correctly.");
                return rlp.multiplyScalar(viewer1.rlpScaleFactor);
            }
        ...
        for(var i = 0; i < this.expt.getNumDetectorPanels(); i++){
            ...
            if (goniometer != null) {
                if (!this.refl.containsRotationAnglesObs() || !this.refl.containsRotationAnglesCal()) panelReflections = this.expt.addAnglesToReflections(panelReflections);
            }
            const pxSize = [
                panelData["pxSize"].x,
                panelData["pxSize"].y
            ...
            var U = null;
            if (this.expt.hasCrystal()) U = this.expt.getCrystalU();
            for(var j = 0; j < panelReflections.length; j++){
                if (containsXYZObs) {
                    const xyzObs = panelReflections[j]["xyzObs"];
                    if (containsWavelengths) wavelength = panelReflections[j]["wavelength"];
                ...
                if (containsXYZCal) {
                    ...
                    if (containsWavelengthsCal) wavelengthCal = panelReflections[j]["wavelengthCal"];
                    if (!wavelengthCal) continue;
        ...
        if (containsXYZObs) {
            if (containsMillerIndices) {
                const pointsObsIndexed = this.createPoints(positionsObsIndexed, ReciprocalLatticeViewer.colors()["reflectionObsIndexed"], this.reflectionSize.value);
            ...
            const pointsObsUnindexed = this.createPoints(positionsObsUnindexed, ReciprocalLatticeViewer.colors()["reflectionObsUnindexed"], this.reflectionSize.value);
        ...
        if (containsXYZCal) {
            const pointsCal = this.createPoints(positionsCal, ReciprocalLatticeViewer.colors()["reflectionCal"], this.reflectionSize.value);
            window.scene.add(pointsCal);
            this.reflPointsCal = [
                pointsCal
            ];
    ...
    addBeam() {
        ...
        const incidentLine = new _three.BufferGeometry().setFromPoints(incidentVertices);
        const incidentMaterial = new _three.LineBasicMaterial({
            color: ReciprocalLatticeViewer.colors()["beam"],
            fog: true,
            depthWrite: false
        });
        const incidentMesh = new _three.Line(incidentLine, incidentMaterial);
        ...
        const outgoingLine = new _three.BufferGeometry().setFromPoints(outgoingVertices);
        const outgoingMaterial = new _three.LineBasicMaterial({
            color: ReciprocalLatticeViewer.colors()["beam"],
            transparent: true,
            opacity: .25,
            fog: true,
        ...
        const outgoingMesh = new _three.Line(outgoingLine, outgoingMaterial);
        this.beamMeshes.push(outgoingMesh);
        window.scene.add(outgoingMesh);
    }
    addSample() {
        const sphereGeometry = new _three.SphereGeometry(ReciprocalLatticeViewer.sizes()["sample"]);
    ...
    addCrystalRLV() {
        ...
        if (!this.expt.hasCrystal()) return;
        ...
        const lineWidth = Math.min(Math.max(avgRLVLength * lineWidthScaleFactor, minLineWidth), maxLineWidth);
        ...
        const labelScaleFactor = Math.max(avgRLVLength * ReciprocalLatticeViewer.sizes()["RLVLabelScaleFactor"], 1);
    ...
    displayDefaultHeaderText() {
        ...
        else this.displayHeaderText(ReciprocalLatticeViewer.text()["default"]);
    }
    ...
    highlightObject(obj) {
        obj.material.color = new _three.Color(ReciprocalLatticeViewer.colors()["highlight"]);
    }
    beamHidden() {
    ...
    updateGUIInfo() {
        function updateReflectionInfo(viewer1) {
            function getDSpacing(arr, idx, viewer1) {
                const rlp = new _three.Vector3(arr[3 * idx] / viewer1.rlpScaleFactor, arr[3 * idx + 1] / viewer1.rlpScaleFactor, arr[3 * idx + 2] / viewer1.rlpScaleFactor);
                return (1 / rlp.length()).toFixed(3);
            }
            if (viewer1.observedIndexedReflsCheckbox.checked) {
                const intersects = window.rayCaster.intersectObjects(viewer1.reflPointsObsIndexed);
                window.rayCaster.setFromCamera(window.mousePosition, window.camera);
                if (intersects.length > 0) for(var i = 0; i < intersects.length; i++){
                    const summary = viewer1.refl.getIndexedSummaryById(intersects[i].index);
                    viewer1.displayHeaderText(summary + " <b>res: </b>" + getDSpacing(viewer1.reflPositionsIndexed, intersects[i].index, viewer1) + " Angstrom");
                }
            }
            if (viewer1.observedUnindexedReflsCheckbox.checked) {
                const intersects = window.rayCaster.intersectObjects(viewer1.reflPointsObsUnindexed);
                window.rayCaster.setFromCamera(window.mousePosition, window.camera);
                if (intersects.length > 0) for(var i = 0; i < intersects.length; i++){
                    const summary = viewer1.refl.getUnindexedSummaryById(intersects[i].index);
                    viewer1.displayHeaderText(summary + " <b>res: </b>" + getDSpacing(viewer1.reflPositionsUnindexed, intersects[i].index, viewer1) + " Angstrom");
                }
            }
        }
        function updateBeamInfo(viewer1) {
            if (viewer1.beamHidden()) return;
            const intersects = window.rayCaster.intersectObjects(viewer1.beamMeshes);
            window.rayCaster.setFromCamera(window.mousePosition, window.camera);
            if (intersects.length > 0) {
                const text = "<b>beam: </b>" + viewer1.expt.getBeamSummary();
                viewer1.displayHeaderText(text);
            }
        }
        function updateCrystalInfo(viewer1) {
            if (viewer1.sampleHidden()) return;
            if (viewer1.expt.getCrystalSummary() === null) return;
            const intersects = window.rayCaster.intersectObjects([
                viewer1.sampleMesh
            ]);
            window.rayCaster.setFromCamera(window.mousePosition, window.camera);
            if (intersects.length > 0) {
                const text = "<b>crystal: </b>" + viewer1.expt.getCrystalSummary();
                viewer1.displayHeaderText(text);
            }
        }
...
function setupScene() {
    ...
    window.scene.fog = new _three.Fog(ReciprocalLatticeViewer.colors()["background"], 500, 8000);
    window.camera = new _three.PerspectiveCamera(45, window.innerWidth / window.innerHeight, 0.01, 10000);
    window.renderer.render(window.scene, window.camera);
    ...
    window.addEventListener("mousedown", function(event) {
        if (event.button == 2) window.viewer.rotateToPos(ReciprocalLatticeViewer.cameraPositions()["defaultWithExperiment"]);
    });
...
window.viewer = new ReciprocalLatticeViewer(new (0, _exptParserJs.ExptParser)(), new (0, _reflParserJs.ReflParser)());
setupScene();
...
(function() {
    ...
    var THREE = root.THREE || has_require && require("2c577d9d017dc749");
